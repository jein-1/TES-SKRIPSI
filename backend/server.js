...
const VAPID_PUBLIC =
  ...
  "BFcCpHwa6FOqP6M02MHCyQPXRKQ5XpaeQ4_ZIoljRHAwtOj7ie7rV3hrCpfs2Y3_7Ze9yP2wviq6btboRyKL8L8";
const VAPID_PRIVATE =
  process.env.VAPID_PRIVATE_KEY ||
  "n8b7fVM4bYhpYzF6YPUvPkgjSoPRv2jR3OcjBedl2Pw";
webpush.setVapidDetails(
  "mailto:admin@aegis.local",
  VAPID_PUBLIC,
  VAPID_PRIVATE,
);
...
const locationLimiter = rateLimit({
  ...
  standardHeaders: true,
  legacyHeaders: false,
  message: { error: "Location rate limit." },
});
...
async function sendPushToAll(payload) {
  ...
  console.log(
    `[Push] Sent to ${pushSubscriptions.size} subscribers (${dead.length} expired removed)`,
  );
}
...
app.get("/api/events", (req, res) => {
  ...
  res.flushHeaders();
  res.write(
    `data: ${JSON.stringify({ type: "INIT", tsunami: tsunamiState })}\n\n`,
  );
  sseClients.add(res);
...
app.post("/api/tsunami", requireAdminKey, async (req, res) => {
  ...
  if (typeof active !== "boolean")
    return res.status(400).json({ error: "Field active harus boolean." });
  tsunamiState = { active, ts: Date.now() };
  // SSE broadcast
  broadcast({
    type: "TSUNAMI",
    active: tsunamiState.active,
    ts: tsunamiState.ts,
  });
...
if (existsSync(distPath)) {
  ...
  app.get("/", (req, res) =>
    res.status(503).json({ error: "App belum di-build." }),
  );
}
